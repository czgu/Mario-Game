        ...
        while(Menu_Loop == 0)
        {
            while(speed_counter > 0)
            {
                    ...
                    for(j=j1;j<=j2;j++){
                        ...
                        if(collide(j*32, i*32,32,32,Mario.xPos,Mario.yPos,Mario.width,Mario.height) && map[i][j]!=0 && Mario.yPos>0  && Mario.life!=0){
                           ...
                           if(map[i][j]==4 && Up_Collision==1){
                               ...
                               animation[0][animation_coin_counter].life=1;
                               animation[0][animation_coin_counter].yPos=(i-1)*32;
                               animation[0][animation_coin_counter].xPos=(j)*32+6;
                               animation[0][animation_coin_counter].down=15;
                               Sound_Effect(1);
                           }
                           if(map[i][j]==13 && Up_Collision==1){
                               ...
                               if(Mario.life==1){ //Mushroom will make you bigger
                                   ...
                                   object[8][mushroom_counter].height = 0;
                                   object[8][mushroom_counter].yPos=(i)*32;
                                   object[8][mushroom_counter].xPos=(j)*32;
                                   object[8][mushroom_counter].move=0;
                                   mushroom_counter++;
                               }
                               else{ //Flower gives the Power to shoot fire ball
                                   object[4][flower_counter].life=1;
                                   object[4][flower_counter].height=0;
                                   object[4][flower_counter].yPos=(i)*32;
                ...
                for(a=9;a>=0;a--){
                    for(b=0;b<100;b++){
                        //height control
                        if((a == 8 || a== 4 )&& object[a][b].life >0)
                        {
                            if(object[a][b].height < 32)
                            {
                                object[a][b].height++;
                            }
                            if(object[a][b].height == 31 && a == 8)
                                    object[a][b].move = 2;
                        }
                ...
                if(Game_level==5 || Game_level==3){ //happens in level 5 and level 3
                    ...
                    if(cannon_counter>100 && Mario.xPos<level[Game_level].ladder_position-400){
            ...
            for(i=0;i<10;i++){
                for(j=0;j<100;j++){
                    if(object[i][j].life>0){
                        ...
                        if(i==8){ //Mushroom
                            masked_blit(ShroomImage,buffer,  0, 0, object[i][j].xPos-x_scroll,object[i][j].yPos, object[i][j].width, object[i][j].height);
                        }
                        ...
                        if(i==4){ //2 colours for the flower
                            if((goomba_sprite_counter/30)%2==0)
                                masked_blit(FlowerImage_2,buffer,  0, 0, object[i][j].xPos-x_scroll,object[i][j].yPos, object[i][j].width, object[i][j].height);
                            else
                                masked_blit(FlowerImage_1,buffer,  0, 0, object[i][j].xPos-x_scroll,object[i][j].yPos, object[i][j].width, object[i][j].height);
                        }
